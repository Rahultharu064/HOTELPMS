...
class CheckInOutService {
     ...
     */
    async checkIn(bookingId, guestData, performedBy = 'system', newRoomId) {
        ...
        if (!['confirmed', 'pending'].includes(booking.status)) {
            throw new ApiError_1.ApiError(constants_1.HttpStatus.BAD_REQUEST, `Cannot check-in booking with status: ${booking.status}`);
        }
        const targetRoomId = newRoomId || booking.roomId;
        const targetRoom = await prisma.room.findUnique({ where: { id: targetRoomId } });
        if (!targetRoom)
            throw new ApiError_1.ApiError(constants_1.HttpStatus.NOT_FOUND, 'Target room not found');
        ...
        if (targetRoom.status === 'occupied') {
            ...
            if (activeOccupant) {
                ...
                if (guestData?.forceCheckIn) {
                    ...
                    throw new ApiError_1.ApiError(constants_1.HttpStatus.CONFLICT, `Room ${targetRoom.roomNumber} is currently occupied by Guest ${activeOccupant.guestId}`);
                }
            }
        }
        // Maintenance / Out of Service check
        if (['maintenance', 'out_of_service'].includes(targetRoom.status)) {
            throw new ApiError_1.ApiError(constants_1.HttpStatus.BAD_REQUEST, `Room ${targetRoom.roomNumber} is currently ${targetRoom.status} and cannot be assigned.`);
        }
        // Cleaning Check
        if (targetRoom.status === 'cleaning' && !guestData?.forceCheckIn) {
            throw new ApiError_1.ApiError(constants_1.HttpStatus.BAD_REQUEST, `Room ${targetRoom.roomNumber} is still being cleaned.`);
        }
     ...
     */
    async checkOut(bookingId, performedBy = 'system', force = false, paymentMethod) {
        const booking = await prisma.booking.findUnique({
            where: { id: bookingId },
            include: {
                room: true,
        ...
        if (!booking)
            throw new ApiError_1.ApiError(constants_1.HttpStatus.NOT_FOUND, 'Booking not found');
        if (booking.status !== 'checked_in') {
            throw new ApiError_1.ApiError(constants_1.HttpStatus.BAD_REQUEST, 'Only checked-in guests can be checked-out');
        }
        // Folio Verification
        const folio = await this.getFolio(bookingId);
        if (folio.balance > 0 && !force) {
            throw new ApiError_1.ApiError(constants_1.HttpStatus.BAD_REQUEST, `Outstanding balance of Rs. ${folio.balance.toLocaleString()} detected.`);
        }
        if (booking.serviceOrders.length > 0 && !force) {
            throw new ApiError_1.ApiError(constants_1.HttpStatus.BAD_REQUEST, 'Pending service orders must be completed or cancelled.');
        }
        return await prisma.$transaction(async (tx) => {
            // 1. Booking Status
     ...
     */
    async markNoShow(bookingId, performedBy = 'system') {
        const booking = await prisma.booking.findUnique({
            where: { id: bookingId },
            include: { room: true }
        });
        if (!booking || !['confirmed', 'pending'].includes(booking.status)) {
            throw new ApiError_1.ApiError(constants_1.HttpStatus.BAD_REQUEST, 'Only pending arrivals can be marked as No-Show');
        }
        return await prisma.$transaction(async (tx) => {
            const updated = await tx.booking.update({
                where: { id: bookingId },
     ...
     */
    async verifyGuestIdentity(bookingId, verificationData) {
        const booking = await prisma.booking.findUnique({
            where: { id: bookingId },
            include: { guest: true }
        });
        ...
        const nameMatches = dbGuest.firstName.toLowerCase().trim() === provided.firstName.toLowerCase().trim() &&
        ...
        if (nameMatches && idMatches) {
            ...
            throw new ApiError_1.ApiError(constants_1.HttpStatus.CONFLICT, 'Guest name does not match the reservation record.');
        }
        else {
            throw new ApiError_1.ApiError(constants_1.HttpStatus.CONFLICT, 'Identification number does not match our records.');
        }
    }
}
