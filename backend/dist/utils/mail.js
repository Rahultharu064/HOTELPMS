...
const preferResend = () => {
    ...
    if (config_1.config.email.provider === 'resend')
        return true;
    if (config_1.config.email.provider === 'smtp')
        return false;
    // auto: Resend in production (Render blocks SMTP), SMTP in local dev
    return config_1.config.isProduction;
};
...
const verifyEmailConfig = async () => {
    ...
    try {
        ...
        console.log(`✅ Email service ready (SMTP ${session.hostname}:${session.port} via IPv4 ${session.connectHost} as ${config_1.config.email.user})`);
        return true;
    }
    catch (error) {
        const err = error;
...
const sendEmail = async (to, subject, html) => {
    ...
    if (activeProvider === 'resend' || (preferResend() && (0, resendProvider_1.isResendConfigured)())) {
    ...
    <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: auto; padding: 0; border: 1px solid #e2e8f0; border-radius: 16px; overflow: hidden; background-color: #ffffff;">
      ...
      <div style="padding: 30px;">
        ...
        <div style="text-align: center; margin-top: 32px;">
          <a href="${config_1.config.frontendUrl}/profile" style="background-color: #14532D; color: white; padding: 14px 28px; text-decoration: none; border-radius: 10px; font-weight: 700; font-size: 14px; display: inline-block;">View Your Booking</a>
        </div>
      </div>
...
const sendStaffWelcomeEmail = async (to, staffName, role, temporaryPassword) => {
    const loginUrl = `${config_1.config.frontendUrl}/admin/login`;
