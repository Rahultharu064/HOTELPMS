...
export function StatCard({ label, value, icon: Icon, color, bg, trend, trendValue, positive }: StatCardProps) {
  return (
    <motion.div 
      whileHover={{ y: -4 }}
      className="group bg-white p-5 rounded-2xl border border-neutral-border/30 hover:border-[#14532D]/30 shadow-sm hover:shadow-xl transition-all duration-300 relative overflow-hidden"
    >
      <div className="flex items-start justify-between relative z-10">
        ...
        <div className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-bold ${positive ? "bg-[#14532D]/10 text-[#14532D]" : "bg-red-50 text-red-600"}`}>
      ...
      <div className="mt-3 relative z-10">
        ...
        <p className={`text-[9px] font-medium mt-1 ${positive ? "text-[#14532D]" : "text-red-600"}`}>{trend}</p>
      </div>
    </motion.div>
  );
}
