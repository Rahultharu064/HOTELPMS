...
/** `roles` restricts a menu item to specific admin roles; omit to show it to everyone with panel access. */
const menuItems = [
...
export function Sidebar({ collapsed, onToggle, mobileOpen, onMobileClose }: AppSidebarProps) {
  ...
  const visibleMenuItems = menuItems.filter(
    (item) => !item.roles || (admin?.role && item.roles.includes(admin.role))
  );

  const sidebarContent = (
    <div className={`flex flex-col h-full bg-primary-dark text-white transition-all duration-300 ${collapsed ? "w-[70px]" : "w-[260px]"}`}>
      {/* Brand Section */}
      <div className="flex items-center gap-3 px-6 py-8 border-b border-white/5 relative overflow-hidden group">
        <div className="absolute top-0 right-0 w-20 h-20 bg-primary-gold/10 rounded-full blur-[30px] -mr-10 -mt-10 transition-all duration-500 group-hover:scale-150 group-hover:bg-primary-gold/20" />
        ...
        {!collapsed && (
          <div className="overflow-hidden relative z-10 transition-all duration-500 delay-100">
            ...
            <p className="text-[8px] font-black uppercase tracking-[0.3em] text-primary-gold mt-1 opacity-90">Management Panel</p>
          </div>
        )}
      </div>
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-[12px] font-bold transition-all duration-300 relative group
                  ${active
                    ? "bg-primary-green text-white shadow-lg shadow-black/10"
                    : "text-white/60 hover:text-white hover:bg-white/5"
                  }`}
              >
                {/* Active Indicator Bar */}
                {active && (
                  <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-5 bg-primary-gold rounded-full shadow-[0_0_8px_var(--color-primary-gold)]" />
                )}

                <item.icon className={`w-4 h-4 flex-shrink-0 transition-transform duration-300 group-hover:scale-110 ${active ? "text-primary-gold" : ""}`} strokeWidth={2} />
                {!collapsed && (
                ...
                {collapsed && (
                  <div className="absolute left-[calc(100%+12px)] top-1/2 -translate-y-1/2 px-3 py-2 bg-foreground text-white text-[10px] font-bold uppercase tracking-widest rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-[60] shadow-2xl whitespace-nowrap">
                    {item.title}
                  </div>
                )}
              </Link>
      ...
      <div className="p-4 border-t border-white/5 bg-black/20">
            ...
            <div className="flex items-center gap-2.5">
              <div className="w-9 h-9 rounded-xl bg-white/5 flex items-center justify-center border border-white/10 transition-all overflow-hidden relative">
                {admin?.avatar ? (
                  ...
                  <div className="text-[9px] font-black text-primary-gold">
              ...
              <div className="overflow-hidden">
                ...
                <p className="text-[9px] font-bold text-primary-gold/80 uppercase mt-0.5 tracking-wider opacity-80">{admin?.role.replace('_', ' ') || 'Online'}</p>
              </div>
            </div>
            <button
              onClick={onToggle}
              className="w-7 h-7 rounded-lg bg-white/5 flex items-center justify-center text-white/40 hover:text-primary-gold hover:bg-white/10 transition-all"
        ...
        {collapsed && (
          <button
            onClick={onToggle}
            className="w-full h-9 mb-3 rounded-xl bg-white/5 flex items-center justify-center text-white/40 hover:text-primary-gold hover:bg-white/10 transition-all"
        ...
        <button
          ...
          className={`flex w-full items-center gap-3 px-4 py-2.5 rounded-xl bg-error-red/10 text-error-red hover:bg-error-red hover:text-white transition-all duration-300 group ${collapsed ? "justify-center" : ""}`}
          title="Logout"
        >
...
export function MobileMenuButton({ onClick }: { onClick: () => void }) {
  return (
    <Button
      onClick={onClick}
      type="button"
      className="lg:hidden h-12 w-12 rounded-2xl bg-white border border-gray-100 text-foreground shadow-xl hover:shadow-2xl transition-all active:scale-95"
    >
