    ...
    return (
      <div className="min-h-[80vh] flex flex-col items-center justify-center gap-6">
         ...
         <p className="text-[11px] font-black uppercase tracking-[0.4em] text-gray-400 animate-pulse text-center">
            ...
            <span className="opacity-40">Synchronizing Global Root Protocols</span>
         </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-12 animate-fade-in pb-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
        <div>
          <h1 className="text-3xl font-black text-[#111827] tracking-tight uppercase">Global Oversight</h1>
          <p className="text-neutral-text-secondary text-[11px] font-black uppercase tracking-[0.2em] mt-2">Real-time Enterprise Intelligence & Control</p>
        </div>
        <div className="flex items-center gap-4">
          <button onClick={fetchDashboardData} className="px-6 py-4 bg-white border border-gray-100 rounded-2xl text-[11px] font-black uppercase tracking-widest text-neutral-text-secondary hover:text-blue-600 hover:bg-gray-50 transition-all shadow-sm flex items-center gap-2">
            <Activity size={16} /> Sync Live Signal
          </button>
          <button className="px-6 py-4 bg-[#111827] text-white rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-blue-600 transition-all shadow-xl shadow-blue-600/10 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {statsConfig.map((s, i) => (
          <motion.div 
            whileHover={{ y: -8 }}
            key={i} 
            className="group bg-white p-8 rounded-[48px] border border-gray-100 hover:border-blue-200 shadow-sm hover:shadow-2xl transition-all duration-500 relative overflow-hidden"
          >
            <div className="flex items-start justify-between relative z-10">
              <div className={`w-16 h-16 rounded-[28px] ${s.bg} ${s.color} flex items-center justify-center group-hover:scale-110 transition-transform duration-500`}>
                <s.icon size={28} strokeWidth={2.5} />
              </div>
              <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-2xl text-[10px] font-black ${s.positive ? "bg-green-50 text-green-600" : "bg-red-50 text-red-600"}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2 space-y-8">
           <div className="flex items-center justify-between">
              <h2 className="text-xl font-black text-[#111827] tracking-tighter uppercase flex items-center gap-3">
                 <TrendingUp className="text-blue-600" size={24} /> Sovereign Transaction Log
              </h2>
                        ...
                        <tr key={log.id} className="hover:bg-gray-50/50 transition-all group cursor-pointer">
                           ...
                           <td className="px-10 py-8">
                              <div className="flex flex-col">
                                ...
                                <span className="text-[10px] font-black text-blue-600">No. {(log as any).room?.roomNumber}</span>
                              </div>
                           </td>
                           ...
                           <td className="px-10 py-8 text-right">
                              <span className={`inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest ${
                                 log.status === 'confirmed' ? 'bg-green-50 text-green-600' : 'bg-amber-50 text-amber-600'
                              }`}>
        ...
        <div className="space-y-12">
           <div className="bg-gradient-to-br from-[#111827] to-[#1F2937] p-12 rounded-[56px] text-white relative overflow-hidden group border border-white/5 shadow-2xl">
              <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500/10 rounded-full blur-[100px] pointer-events-none group-hover:scale-150 transition-transform duration-[2000ms]" />
              <ShieldCheck className="text-blue-500 mb-8" size={32} />
              <h3 className="text-3xl font-black mb-4 leading-tight">Sovereign Node<br /><span className="text-white/40 italic font-medium">Control Protocol</span></h3>
              ...
              <button className="w-full py-5 bg-blue-600 text-white rounded-[32px] text-[12px] font-black uppercase tracking-widest hover:bg-white hover:text-blue-600 transition-all shadow-2xl shadow-blue-600/20 active:scale-95">
                 Engage Master Auth
              </button>
           </div>
           
           <div className="p-10 bg-white rounded-[48px] border border-gray-100 shadow-sm relative overflow-hidden group">
              ...
              <div className="space-y-4">
                 <input 
                    ...
                    className="w-full px-6 py-4 bg-gray-50 border border-transparent focus:border-blue-600/20 focus:bg-white rounded-2xl text-[12px] font-medium outline-none transition-all shadow-inner"
                 />
                 <button className="w-full py-4 text-[10px] font-black uppercase tracking-widest text-blue-600 hover:bg-blue-50 transition-colors rounded-2xl">
                    Query Data Core
                 </button>
              </div>
           </div>
