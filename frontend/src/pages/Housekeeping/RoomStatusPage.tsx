  ...
  return (
    <div className="space-y-10 animate-fade-in pb-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
        <div>
          <h1 className="text-3xl font-black text-[#111827] tracking-tight">Room Status Inventory</h1>
          <p className="text-gray-400 text-[11px] font-bold uppercase tracking-widest mt-2">Track real-time room occupancy and cleaning cycles</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {filteredRooms.map((room) => {
          ...
          return (
            ...
            >
              <div className={`absolute top-8 right-10 w-2.5 h-2.5 rounded-full ${
                isClean ? 'bg-emerald-500' : isDirty ? 'bg-amber-500' : isOccupied ? 'bg-blue-500' : 'bg-red-500'
              }`} />
              
              <div className={`w-20 h-20 rounded-[32px] flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-500 ${
                isClean ? 'bg-emerald-50 text-emerald-600' : isDirty ? 'bg-amber-50 text-amber-600' : isOccupied ? 'bg-blue-50 text-blue-600' : 'bg-red-50 text-red-600'
              }`}>
